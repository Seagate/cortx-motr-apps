...
// #cgo CFLAGS: -I/usr/include/mero -fPIC -DM0_EXTERN=extern -DM0_INTERNAL=
...
// #include "clovis/clovis_internal.h" /* m0c_pools_common */
// #include "clovis/clovis_idx.h"      /* M0_CLOVIS_IDX_DIX */
//
// struct m0_clovis          *instance = NULL;
// struct m0_clovis_container container;
// struct m0_clovis_realm     uber_realm;
// struct m0_clovis_config    conf = {};
// struct m0_idx_dix_config dix_conf = {};
//
// int m0_open_entity(struct m0_clovis_entity *entity)
// {
//         int           rc;
//         struct m0_clovis_op *op = NULL;
//
//         rc = m0_clovis_entity_open(entity, &op);
//         if (rc != 0)
//                 goto cleanup;
//
...
func Init() {
    ...
    C.conf.cc_is_oostore     = true
    C.conf.cc_local_addr     = C.CString(*localEP)
    C.conf.cc_ha_addr        = C.CString(*haxEP)
    C.conf.cc_profile        = C.CString(*profile)
    C.conf.cc_process_fid    = C.CString(*procFid)
    C.conf.cc_tm_recv_queue_min_len =    64
    C.conf.cc_max_rpc_msg_size      = 65536
    C.conf.cc_idx_service_id  = C.M0_CLOVIS_IDX_DIX;
    C.dix_conf.kc_create_meta = false;
    C.conf.cc_idx_service_conf = unsafe.Pointer(&C.dix_conf)

    rc := C.m0_clovis_init(&C.instance, &C.conf, true)
    if rc != 0 {
        log.Panicf("m0_clovis_init() failed: %v", rc)
    }

    C.m0_clovis_container_init(&C.container, nil,
			       &C.M0_CLOVIS_UBER_REALM, C.instance)
...
func (mio *Mio) objNew(id string) (err error) {
    ...
    mio.obj = (*C.struct_m0_clovis_obj)(
                      C.calloc(1, C.sizeof_struct_m0_clovis_obj))
    return nil
}
...
func (mio *Mio) Open(id string) (err error) {
    ...
    C.m0_clovis_obj_init(mio.obj, &C.container.co_realm, &mio.objID, 1)
...
func (mio *Mio) Create(id string, sz uint64) error {
    ...
    C.m0_clovis_obj_init(mio.obj, &C.container.co_realm, &mio.objID, lid)

    var op *C.struct_m0_clovis_op
    rc := C.m0_clovis_entity_create(pool, &mio.obj.ob_entity, &op)
    if rc != 0 {
        return fmt.Errorf("failed to create object: %d", rc)
    }
    C.m0_clovis_op_launch(&op, 1)
    rc = C.m0_clovis_op_wait(op, bits(C.M0_CLOVIS_OS_FAILED,
				      C.M0_CLOVIS_OS_STABLE), C.M0_TIME_NEVER)
    if rc == 0 {
        rc = C.m0_clovis_rc(op)
    }
    C.m0_clovis_op_fini(op)
    C.m0_clovis_op_free(op)
...
func (mio *Mio) getOptimalBlockSz(bufSz int) (bsz, gsz int) {
    ...
    usz := int(C.m0_clovis_obj_layout_id_to_unit_size(C.ulong(mio.objLid)))
    pa := &pver.pv_attr
    gsz = usz * int(pa.pa_N)
...
func (mio *Mio) doIO(i int, opcode uint32) {
    defer mio.wg.Done()
    var op *C.struct_m0_clovis_op
    C.m0_clovis_obj_op(mio.obj, opcode,
		       &mio.ext[i], &mio.buf[i], &mio.attr[i], 0, &op)
    C.m0_clovis_op_launch(&op, 1)
    rc := C.m0_clovis_op_wait(op, bits(C.M0_CLOVIS_OS_FAILED,
                                C.M0_CLOVIS_OS_STABLE), C.M0_TIME_NEVER)
    if rc == 0 {
...
func (mio *Mio) Write(p []byte) (n int, err error) {
    ...
    for ; left > 0; left -= bs {
        ...
        mio.wg.Add(1)
        go mio.doIO(slot.idx, C.M0_CLOVIS_OC_WRITE)
        off += bs
        mio.off += uint64(bs)
    }
...
func (mio *Mio) Read(p []byte) (n int, err error) {
    ...
    for ; left > 0 && mio.off < ObjSize; left -= bs {
        ...
        mio.wg.Add(1)
        go mio.doIO(slot.idx, C.M0_CLOVIS_OC_READ)
        if mio.minBuf != nil {
